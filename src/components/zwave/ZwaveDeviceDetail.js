...
class ZwaveDeviceDetail extends Component {
  ...
  render(){
    ...
    const parameters = data.parameters.map(parameter => (
      <tr
        key={`${data.moduleId}${parameter.valueId}`}
        style={{ cursor: 'pointer' }}>
        <td onClick={() => onRowClick(data.moduleId, parameter.valueId)}>
          {parameter.name}
        </td>
        <td onClick={() => onRowClick(data.moduleId, parameter.valueId)}>
          {parameter.valueId}
        ...
        <td>
          <button
            ...
            onClick={() => {
              const modalName = `ZwaveEnablePollModal${data.moduleId}${parameter.valueId}`;
              const modal = document.getElementById(modalName);
              M.Modal.getInstance(modal).open();
            }}>
          ...
          { parameter.polled ?
            <button
              ...
              onClick={() => {
                const modalName = `ZwaveDisablePollModal${data.moduleId}${parameter.valueId}`;
                const modal = document.getElementById(modalName);
                M.Modal.getInstance(modal).open();
              }}>
              <i className="material-icons">update</i>
